...
export function getCardPosition(stack, card) {
  const selectedIndex = stack.indexOf(card);
...
export function doesStackIncludeCard(stack, card) {
  const item = stack.includes(card);
  return item
}
...
export function isEachCardEven(stack) {
  const even = stack.every(card => card % 2 === 0);
  return even;
}
...
export function doesStackIncludeOddCard(stack) {
  const odd = stack.some(card => card % 2 === 1);
  return odd;
}
...
export function getFirstOddCard(stack) {
  const firstOddItem = stack.find(card => card % 2 ===1)
  return firstOddItem
}
...
export function getFirstEvenCardPosition(stack) {
  const firstOddIndex = stack.findIndex((card) => card % 2 == 0)
